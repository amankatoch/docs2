...
REDIPS.table = (function () {
	...
	onmousedown = function (el, flag, type) {
		...
		get_tables = function (el) {
			var arr = [],	// result array
				...
				i;			// loop variable
			// collect table nodes
      console.log($(el[0]).contents());
      
			nodes = $(el[0]).contents().find('table');
      ...
      // open node loop and push to array
		...
		if (typeof(el) === 'string') {
			if (type === 'classname') {
				...
				tables = get_tables(document.getElementsByTagName('iframe'));
			...
			// first parameter is string and that should be id of container or id of a table
				...
				el = document.getElementsByTagName('iframe')[0].contentDocument.getElementById(el);
	...
	row = function (table, mode, index) {
		...
		if (typeof(table) !== 'object') {
			table = document.getElementsByTagName('iframe')[0].contentDocument.getElementsByClassName(table)[0];
		}
